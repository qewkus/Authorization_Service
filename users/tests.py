...
from users.services import AuthService, SMSService
...
class ProfileViewTest(APITestCase):
    def setUp(self):
        self.user = CustomUser.objects.create(
            phone_number="+79991234567", invite_code="ABC123"
        )
...
class ActivateReferralCodeViewTest(APITestCase):
    def setUp(self):
        self.user = CustomUser.objects.create(
            phone_number="+79991234567", invite_code="BRPDI5"
        )
        self.referral = CustomUser.objects.create(
            phone_number="+79997654321", invite_code="CSK123"
        )
        self.client.force_authenticate(user=self.user)
